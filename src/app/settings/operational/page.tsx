...
import { manageOperationalProfile } from '@/lib/auth-service';
...
export default function OperationalProfilesPage() {
  ...
  const [formData, setFormData] = useState<Partial<OperationalProfile>>({});

  useEffect(() => {
    const q = query(collection(db, 'operationalProfiles'), orderBy('versionNumber', 'desc'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const profileList = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as OperationalProfile));
  ...
  const handleSave = async (id?: string) => {
    try {
      if (id) {
        await manageOperationalProfile('update', formData, id);
        setEditingId(null);
      } else {
        ...
        await manageOperationalProfile('create', { 
          ...formData, 
          status: 'draft', 
  ...
  const handlePublish = async (id: string) => {
    ...
    try {
      await manageOperationalProfile('publish', {}, id);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Operational Profiles</h1>
          <p className="text-sm text-gray-500">Manage versioned operational configurations</p>
        </div>
        <button
